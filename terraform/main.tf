...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      CORS_ORIGINS     = (var.use_custom_domain && var.environment == "prod") ? "https://${var.root_domain},https://www.${var.root_domain}" : "https://${aws_cloudfront_distribution.main.domain_name}"
...
# Explicit OPTIONS routes so Lambda handles preflight and returns CORS headers
# (API Gateway CORS alone can miss headers in some cases)
...
resource "aws_apigatewayv2_route" "options_health" {
  api_id    = aws_apigatewayv2_api.main.id
